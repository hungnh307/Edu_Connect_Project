    ...
    public static class AnimationClipExtensions
    {
        ...
        private static void BindOrUnbindCallback(this AnimationClip clip, GameObject animEvtReceiverObject, float atPosition, Action callback, bool bind)
        {
            ...
            if (bind)
            {
                ...
                eventReceiver.RegisterTimelineCallback(atPosition, clip.name, callback);
                clip.AddEventIfNotExists(OnTimelineEventRaisedMethodName, atPosition, clip.name, atPosition);
            }
            else
            {
                if (eventReceiver == null)
                ...
                var lastCallbackForPositionRemoved = eventReceiver.UnregisterTimelineCallback(atPosition, clip.name, callback);
                if (lastCallbackForPositionRemoved)
                {
                    clip.RemoveEvent(OnTimelineEventRaisedMethodName, atPosition, clip.name, atPosition);
                }
            }
        }

        private static void AddEventIfNotExists(this AnimationClip clip, string methodName, float floatParameter, string stringParameter, float time)
        {
            var clipAnimationEvents = clip.events;
            var animationEvent = Array.Find(clipAnimationEvents,
                e => e.functionName == methodName && e.floatParameter == floatParameter && e.stringParameter == stringParameter && e.time == time);
        ...
        private static void RemoveEvent(this AnimationClip clip, string methodName, float floatParameter, string stringParameter, float time)
        {
            var clipAnimationEvents = clip.events;
